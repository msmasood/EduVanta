...
export function TeacherAttendancePage() {
  ...
  const allRows = React.useMemo(() => {
    ...
    return mapTeacherAttendanceRows(records, teachers, departments);
  }, [attendanceQuery.data, teachersQuery.data, departmentsQuery.data]);
  ...
  const summary = React.useMemo(() => {
    const records = attendanceQuery.data?.data ?? [];
    return computeAttendanceSummary(records.filter((r) => r.entityType === "teacher"));
  ...
  const calendarDays = React.useMemo(() => {
    ...
    return buildAttendanceCalendarDays(
      records.filter((r) => r.entityType === "teacher"),
      now.getFullYear(),
      now.getMonth() + 1
    );
  }, [attendanceQuery.data]);

  function handleEdit(row: TeacherAttendanceRow) {
    toast.info(`Edit status for ${row.teacherName} on ${row.date}.`);
  }

  function handleNotes(row: TeacherAttendanceRow) {
    setNotesDialog({
      open: true,
      recordId: row.id,
      entityName: row.teacherName,
      date: row.date,
    ...
    <div className="space-y-6" data-testid="teacher-attendance-page">
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-muted-foreground">
          View and manage daily attendance records for all teachers.
        </p>
      </div>
